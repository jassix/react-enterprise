...
@category Union
*/
export type SharedUnionFieldsDeep<
	Union,
...
> = ApplyDefaultOptions<
	...
	Options
> extends infer OptionsWithDefaults extends
	Required<SharedUnionFieldsDeepOptions>
...
*/
type SharedObjectUnionFieldsDeep<
	Union,
	Options extends Required<SharedUnionFieldsDeepOptions>,
> = keyof Union extends infer Keys // `keyof Union` can extract the same key in union type, if there is no same key, return never.
...
*/
type SharedArrayUnionFieldsDeep<
	Union extends UnknownArray,
	Options extends Required<SharedUnionFieldsDeepOptions>,
> = SetArrayAccess< // Restore the readonly modifier of the array.
	...
	? // Rule 1: If all the arrays in the union have non-fixed lengths,
		...
		: // Build the fixed-length tuple recursively.
			InternalSharedArrayUnionFieldsDeep<
				...
				[
					...ResultTuple,
					SharedUnionFieldsDeep<Union[ResultTuple["length"]], Options>,
				]
			>
