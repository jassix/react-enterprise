import type { ArrayTail as _ArrayTail } from "./array-tail";
...
type Writable<TArray extends UnknownArray> = {
...
> = { // Case in rule 1: The destination contains the key but the source doesn't.
	...
	[Key in keyof Source as Key extends keyof Destination
		? Key
		: never]: MergeDeepRecordProperty<Destination[Key], Source[Key], Options>;
};
...
```
*/
type PickRestType<Type extends UnknownArrayOrTuple> =
...
type TypeNumberOrType<Type extends UnknownArrayOrTuple> =
...
type PickRestTypeFlat<Type extends UnknownArrayOrTuple> = TypeNumberOrType<
	PickRestType<Type>
>;
...
*/
type ShouldSpread<Options extends MergeDeepInternalOptions> =
...
> = ShouldSpread<Options> extends true
	? Array<
			| Exclude<Destination, undefined>[number]
			| Exclude<Source, undefined>[number]
		>
	: Source; // 'replace'
...
> = Destination[number] extends UnknownArrayOrTuple
	? Source[number] extends UnknownArrayOrTuple
		? Array<
				MergeDeepArrayOrTupleRecursive<
					Destination[number],
					Source[number],
					Options
				>
		...
		? Source[number] extends UnknownRecord
			? Array<
					SimplifyDeepExcludeArray<
						MergeDeepRecord<Destination[number], Source[number], Options>
					>
				>
			: DoMergeArrayOrTuple<Destination, Source, Options>
		: DoMergeArrayOrTuple<Destination, Source, Options>;
...
Internal options.
*/
type MergeDeepInternalOptions = Merge<
	MergeDeepOptions,
			...
			: Destination extends UnknownArrayOrTuple
				? Source extends UnknownArrayOrTuple
					? MergeDeepArrayOrTuple<
							Destination,
							Source,
							DefaultMergeDeepOptions<Options>
						>
					: never
				: never
>;
