...
export type Sum<A extends number, B extends number> = number extends A | B // Handle cases when A or B is the actual "number" type
	? number
	: // Handle cases when A and B are both +/- infinity
		A extends B & (PositiveInfinity | NegativeInfinity)
		? A // A or B could be used here as they are equal
...
> = AreNegative extends [false, false]
	...
	: AreNegative extends [true, true]
		...
		: // When the signs are different we can subtract the absolute values, remove the sign
			...
			NumberAbsolute<
					Subtract<NumberAbsolute<A>, NumberAbsolute<B>>
				> extends infer Result extends number
			? TupleMax<
					[NumberAbsolute<A>, NumberAbsolute<B>]
				> extends infer Max_ extends number
				? Max_ extends A | B
