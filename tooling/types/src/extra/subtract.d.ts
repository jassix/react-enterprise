import type { BuildTuple, NumberAbsolute, ReverseSign } from "./internal/index";
...
export type Subtract<A extends number, B extends number> = number extends A | B // Handle cases when A or B is the actual "number" type
	? number
...
> = AreNegative extends [false, false]
	...
	: AreNegative extends [true, true]
		...
		: // When the signs are different we can add the absolute values and then reverse the sign if A < B
			[
					...BuildTuple<NumberAbsolute<A>>,
					...BuildTuple<NumberAbsolute<B>>,
...
*/
type SubtractPositives<A extends number, B extends number> = LessThan<
	A,
	B
> extends true
...
type SubtractIfAGreaterThanB<
	A extends number,
	B extends number,
> = BuildTuple<A> extends [...BuildTuple<B>, ...infer R] ? R["length"] : never; // This is where we always want to end up and do the actual subtraction
