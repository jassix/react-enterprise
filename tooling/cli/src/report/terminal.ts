...
import { type Colors, type ColorKey, createColors } from "./colors";
...
function renderResult(result: CheckResult, colors: Colors): string[] {
	...
	const detail = result.status === "ok"
		? (result.detail ? ` ${colors.wrap("dim", result.detail)}` : "")
		: ` ${result.detail}`;
	const lines = [
		`  ${colors.wrap(sym.color, sym.glyph)} ${result.name}${detail}`,
	];
