...
Full reference for the Table half of the `tanstack` skill. For the high-level overview and which-to-load decision, see [`SKILL.md`](./SKILL.md).
...
> When pairing with TanStack Query, drop the `useEffect` and put `sorting`/`columnFilters`/`pagination` directly into the query key — the cache will refetch automatically when state changes.
