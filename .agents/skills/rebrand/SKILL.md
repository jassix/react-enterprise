---
name: rebrand
description: Rebrand this React monorepo template for a new project. Renames the `@repo/*` package scope, the `repo` CLI bin name, branding strings, and docs to match the user's organization. Use once, right after cloning the template, when the user says things like "adapt this template", "rebrand to our company", "set up for Acme", or runs `repo adapt`.
version: 1.0.0
user-invocable: true
argument-hint: "[new-scope (e.g. @acme)] [new-cli-name (e.g. acme)]"
---

# rebrand

One-shot skill for turning this template into a real project. The template ships with `@repo/*` as the package scope and `repo` as the CLI bin name — both are placeholders meant to be replaced on first use.

**Delete this skill after running it.** It has no repeat use in a single project.
...
```bash
rg '@repo' --glob '!bun.lock' --glob '!.agents/skills/rebrand/**' -l | head -5
...
rg '@repo' --glob '!bun.lock' --glob '!.agents/skills/rebrand/**'
...
This skill exists only to run once. Delete it:

```bash
rm -rf .agents/skills/rebrand
rm .claude/skills/rebrand
```
