...
Comprehensive guide for React and Next.js applications, designed for AI agents and LLMs. Contains 40+ rules across 9 categories — eight performance categories prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns), plus a project-local code style category for component declarations and props typing. Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
9. [Code Style & Conventions](#9-code-style--conventions) — **LOW**
   - 9.1 [Declare Components as `const` with `FC<Props>`](#91-declare-components-as-const-with-fcprops)
   - 9.2 [Use `interface` for Component Props and Extend via `ComponentProps`](#92-use-interface-for-component-props-and-extend-via-componentprops)
...
Project-wide style rules for component declarations and props typing. Not a performance category — these enforce a consistent shape across the codebase so components are easier to read, refactor, and forward props through.

### 9.1 Declare Components as `const` with `FC<Props>`
...
- Pairs with [Use `interface` for Component Props and Extend via `ComponentProps`](#92-use-interface-for-component-props-and-extend-via-componentprops).
...
```tsx
const Card: FC<CardProps> = ({ children, ...props }) => <div {...props}>{children}</div>;
...
### 9.2 Use `interface` for Component Props and Extend via `ComponentProps`
